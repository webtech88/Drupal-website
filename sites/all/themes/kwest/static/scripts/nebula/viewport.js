...
define('nebula/viewport', ['settings', 'nebula/signal'], function(settings, Signal) {
    ...
    function _is(alias) {
        ...
        if (alias in VIEWPORTS)
            return alias === active;
        else if (alias === 'viewport-portrait')
            return isPortrait();
        else if (alias === 'viewport-landscape')
            return isLandscape();
        throw new Error('Unknown viewport alias: ' + alias);
    }

    function is(aliases, all) {
        if (!isEnabled())
        ...
        /*
         * If `all` == `true`, returns `true` if ALL of the aliases are currently active (makes sense for `aliases` like `720 viewport-portrait`).
         * If `all` == `false`, returns `true` if ANY of the aliases is currently active.
         * `aliases` must be a string with any viewport names defined in settings, separated by space.
         */
        aliases = aliases.split(' ');
        for (i = 0; i < aliases.length; i++) {
            is_alias = _is(aliases[i]);
            if (all ^ is_alias)
                return is_alias;
        }
